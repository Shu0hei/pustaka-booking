...
class Buku extends CI_Controller
{
  ...
  public function index()
  {
    ...
    $data['buku'] = $this->ModelBuku->joinNamaKategori();
  ...
  public function createKategori()
  {
    ...
    if ($this->form_validation->run() == FALSE) {
      $this->load->view('buku/tambah_kategori.php');
    } else {
      $data = array(
        'nama_kategori' => $this->input->post('nama_kategori')
      );

      if ($this->ModelKategori->createKategori($data)) {
        $this->session->set_flashdata('message', 'Kategori berhasil ditambahkan');
        redirect('buku/kategori');
      } else {
        $this->session->set_flashdata('message', 'Terjadi kesalahan saat menambahkan kategori');
        redirect('buku/createKategori');
      }
    }
  }
